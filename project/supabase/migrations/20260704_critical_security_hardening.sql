/*
  ...
  - Public checkout no longer accepts arbitrary discount amounts.
  - Public product reads go through a whitelisted view.
  - Direct product table reads are restricted to staff for authenticated admin use.
  - POS cannot sell unavailable catalog products.
*/
...
BEGIN
  ...
  LOOP
    ...
    VALUES (v_order.id, v_product.id, v_product.name, v_quantity, v_unit_price);
