/*
  # Ajout du statut annulé et correction du calcul du chiffre d'affaires
  
  1. Modifications
    - Ajout du statut 'cancelled' à l'enum order_status
    - Modification de la fonction get_sales_stats pour ne compter que les commandes confirmées
*/
...
BEGIN
  -- Calculer les totaux uniquement pour les commandes confirmées ou livrées
  SELECT 
  ...
  WITH monthly_stats AS (
    ...
    WHERE o.status IN ('confirmed', 'shipped', 'delivered')
    AND o.created_at >= date_trunc('month', current_date - interval '1 year')
    GROUP BY month
  ),
  prev_month_stats AS (
    SELECT
