/*
  # Correction du calcul des ventes
  
  1. Modifications
    - Correction du calcul du total des ventes pour inclure uniquement les commandes confirmées et les ventes en caisse
    - Ajout d'un statut 'cancelled' pour les commandes annulées
    - Optimisation des requêtes avec des index
*/

-- Supprimer et recréer l'enum order_status
DROP TYPE IF EXISTS order_status CASCADE;
CREATE TYPE order_status AS ENUM ('pending', 'confirmed', 'shipped', 'delivered', 'cancelled');

-- Recréer la fonction get_sales_stats avec le calcul corrigé
...
BEGIN
  ...
  WITH all_sales AS (
    ...
    -- Ventes en caisse (toujours considérées comme confirmées)
    SELECT
      o.id as order_id,
      o.email,
      o.created_at,
    ...
    WHERE o.status != 'cancelled' -- Exclure les ventes annulées
    GROUP BY o.id, o.email, o.created_at
  )
  SELECT 
