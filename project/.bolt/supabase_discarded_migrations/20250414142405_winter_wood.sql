/*
  # Fix User Creation and Profiles
  
  1. Changes
    - Drop and recreate profiles table with proper structure
    - Create function to handle user creation with profile
...
BEGIN
  ...
  -- Create cashier user
  IF NOT EXISTS (SELECT 1 FROM auth.users WHERE email = 'caissier@sphere-office.com') THEN
    PERFORM create_user_with_profile('caissier@sphere-office.com', 'Caissier123', 'cashier');
  END IF;
END $$;